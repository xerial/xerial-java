...
/**
 * Variable length number representation. When the most significant bit (MSB) of
 * a byte is 0, it means the termination of the value, when the MSB of a byte is
 * 1, its following bytes must be read to compute the value.
 * 
 * 0XXXXXXX : [0, 127 (= M1 - 1)] 1XXXXXXX 0XXXXXXX : [M1, M1 + 127 * 2^7 + 127
 * (= M2-1)] 1XXXXXXX 1XXXXXXX 0XXXXXXX : [M2, M2 + 127 * 2^14 + 127 * 2^7 +
 * 127]
 * 
 * [0, 0 + (2 ^ 7 - 1) = 127] [128, 128 + (2^14 - 1) = 16511] [16512, 16512 +
 * (2^21 - 1) = 2113663]
 * 
 * @author leo
 * 
...
public class VariableLengthInteger {
    ...
    public VariableLengthInteger(byte[] byteArray, int offset) throws XerialException {
    ...
    public static int byteSize(byte[] buffer, int offset) throws XerialException {
        int byteLength = 1;
        int index = offset;
        while ((buffer[index++] & 0x80) != 0) {
            byteLength++;
            if (byteLength > upperBoundByByteSize.length) {
                throw new XerialException(XerialErrorCode.InvalidFormat,
                        "cannot read the variable length integer whose bite size is larger than "
                                + upperBoundByByteSize.length);
            }
        }
        return byteLength;
    ...
    public static int readFrom(byte[] buffer, int offset) throws XerialException {
        int value = 0;
        ...
        if (byteLength > Buffer.INT_SIZE) {
            if (!(byteLength == (Buffer.INT_SIZE + 1) && (buffer[offset] > 0x87))) // 1000 0111
                throw new XerialException(XerialErrorCode.INVALID_INPUT,
                        "value larger than 2^31-1 cannot be read");
        }
    ...
    private void translate(byte[] byteArray, int offset) throws XerialException {
        int index = offset;
        while (index < byteArray.length) {
            if ((byteArray[index] & 0x80) == 0) {
